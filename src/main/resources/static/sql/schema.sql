...
ALTER TABLE Employee ADD CONSTRAINT Employee_Warehouse
    FOREIGN KEY (Warehouse_ID)
    REFERENCES Warehouse (ID);

-- Reference: Item_Purchase_Item (table: Item_Sale)
ALTER TABLE Item_Sale ADD CONSTRAINT Item_Purchase_Item
    FOREIGN KEY (Item_ID)
    REFERENCES Item (ID);

-- Reference: Item_Purchase_Purchase (table: Item_Sale)
ALTER TABLE Item_Sale ADD CONSTRAINT Item_Purchase_Purchase
    FOREIGN KEY (Sale_ID)
    REFERENCES Sale (ID);

-- Reference: Purchase_Employee (table: Sale)
ALTER TABLE Sale ADD CONSTRAINT Purchase_Employee
    FOREIGN KEY (Salesman_ID)
    REFERENCES Employee (ID);

-- Reference: Table_9_Item (table: Warehouse_Items)
ALTER TABLE Warehouse_Items ADD CONSTRAINT Table_9_Item
    FOREIGN KEY (Item_ID)
    REFERENCES Item (ID);

-- Reference: Table_9_Warehouse (table: Warehouse_Items)
ALTER TABLE Warehouse_Items ADD CONSTRAINT Table_9_Warehouse
    FOREIGN KEY (Warehouse_ID)
    REFERENCES Warehouse (ID);
